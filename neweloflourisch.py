...
def main():
    ...
    for date in dates:
        x += 1
        print(f'So far {x} rallys have sorted top 10')
        if x == 5000:
            break
...
def finder(leaderboard, data, date):
    for seat in ["driver", "codriver"]:
        ...
        for name in data[seat].keys():
            try:
                newElo = data[seat][name]["elo"]["history"][date]["total_elo"]
            except:
                newElo = 0
